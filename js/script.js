    ...
    .then(resJson=>{
        ...
        resJson.teams.forEach(team=>{
            ...
            <li class="collection-item avatar">
                ...
                <a href="#" data-target="modal1" data-id="${team.id}"class=" modal-trigger secondary-content"><i class="material-icons" data-id="${team.id}">info</i></a>
            </li>
            `
        });
...
function showTeamInfo(id){
    ...
    .then(detail=>{
        ...
        <div class="row">
        <div class="col s5">
        <img src="${detail.crestUrl}" alt="" style="margin-top:20px;" width="280px">
        </div>
        <div class="col s6">
        ...
        <p> Berdiri Sejak : ${detail.founded}</p>
        <p> Warna Club : ${detail.clubColors}</p>
        <p> Stadion : ${detail.venue}</p>
        </div>
        </div>
        `;
        document.getElementById('modals').innerHTML = details;
    })
}
